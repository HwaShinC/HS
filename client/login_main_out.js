(function (wHandle, wjQuery) {
     ...
     */
    var SKIN_URL = "http://agark.kr/skins/login/";//skins folder
    ...
    function gameLoop() {
        ...
        wjQuery("#helloDialog").show();
        wjQuery("#matchResults").hide();
...
////
    ...
    function drawGameScene() {
        ...
        if(noticeOn) {
            ncCanvas && ncCanvas.width && ctx.drawImage(ncCanvas, canvasWidth / 2 - ncCanvas.width / 2, 10); // draw Leader Board
        } else {
            ncCanvas = null;
        }
        ...
        if (0 != userScore) {
            ...
            scoreText.setValue('점수: ' + ~~(userScore / 100));
            c = scoreText.render();
            a = c.width;
            ctx.globalAlpha = .2;
            ctx.fillStyle = '#193300';
    ...
    function drawNoticeBoard() {
        ncCanvas = null;
        if (null != teamScores || 0 != noticeBoard.length)
            if (null != teamScores || showName) {
                ncCanvas = document.createElement("canvas");
                var ctx = ncCanvas.getContext("2d"),
                    boardLength = 60;
                boardLength = null == teamScores ? boardLength + 30 * noticeBoard.length : boardLength + 180;
                var scaleFactor = Math.min(0.22*canvasHeight, Math.min(200, .3 * canvasWidth)) / 200;
                ncCanvas.width = 600 * scaleFactor;
                ncCanvas.height = boardLength * scaleFactor;

                ctx.scale(scaleFactor, scaleFactor);
                ctx.globalAlpha = .4;
                ctx.fillStyle = "#FF0000";
                ctx.fillRect(0, 0, 600, boardLength);

                ctx.globalAlpha = 1;
                ctx.fillStyle = "#FFFFFF";
                var c = "❀[Agark 서버 관리자]❀";
                ctx.font = '35px NanumGothic, "Malgun Gothic", Ubuntu';
                ctx.fillText(c, 300 - ctx.measureText(c).width / 2, 40);
                var b;
                if (null == teamScores) {
                    for (ctx.font = '25px NanumGothic, Malgun Gothic', b = 0; b < noticeBoard.length; ++b) {
                        c = noticeBoard[b] || "";
                        if (!showName) {
                            (c = "Agark.kr");
                        }
                        ctx.fillStyle = "#66FFFF";
                        ctx.fillText(c, 300 - ctx.measureText(c).width / 2, 70 + 30 * b);
                    }
                }
            }
    }
    ...
    function drawLeaderBoard() {
        ...
        if (null != teamScores || 0 != leaderBoard.length)
            if (null != teamScores || showName) {
                ...
                ctx.fillStyle = "#FFFFFF";
                var c = "리더보드";
                ctx.font = '30px NanumGothic, Malgun Gothic';
                ctx.fillText(c, 100 - ctx.measureText(c).width / 2, 40);
                var b;
                if (null == teamScores) {
                    for (ctx.font = "20px NanumGothic, Malgun Gothic", b = 0; b < leaderBoard.length; ++b) {
                        c = leaderBoard[b].name || "Agark.kr";
                        if (!showName) {
                            (c = "An unnamed cell");
                        }
    ...
    var nCanvas, ctx, mainCanvas, ncCanvas, lbCanvas, chatCanvas, canvasWidth, canvasHeight, qTree = null,
        ws = null,
        nodeX = 0,
        nodeY = 0,
        nodesOnScreen = [],
        ...
        render: function () {
            ...
            if (this._dirty) {
                ...
                var canvas = this._canvas,
                    ...
                    scale = this._scale,
                    fontsize = this._size,
                    font = fontsize + 'px  NanumGothic, Malgun Gothic';
                ctx.font = font;
                var h = ~~(.2 * fontsize);
...
//console.log(knownNameDict);
    ...
    setInterval(function(){
        var myScore = (~~(userScore / 100)) - level*100; 
        //wjQuery("#levelScore").text(myScore);
        wjQuery("#levelBar").css('width', myScore + '%');
        if(myScore > 100){
            level++;
            wjQuery("#lvl").text(level);
        }
        lastP = myScore;
    },10);
